...
const int PIN_MOSFET     = 6;   // MOSFET ゲート (PWM 出力)
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (c == 's' || c == 'S') {
      motorPWM = 0;
      // PWM を 0 にし、さらにデジタルで強制 LOW にしてゲートを確実に落とす
      analogWrite(PIN_MOSFET, 0);
      pinMode(PIN_MOSFET, OUTPUT);
      digitalWrite(PIN_MOSFET, LOW);
      currentState = STATE_IDLE;
      modeManual = false;
      Serial.println("EMERGENCY STOP: motor PWM=0, gate forced LOW");
    }
  }
  ...
  if (powerPressed != powerState) {
    if (now - lastDebouncePower >= DEBOUNCE_DELAY) {
      ...
      if (powerState) {
        ...
        Serial.println("Power ON: manual WEAK");
      } else {
        // 離されたら自動モードに戻す
        modeManual = false;
        currentState = STATE_IDLE;
        sensorValid = false;
        Serial.println("Power OFF: go to IDLE");
      }
    }
  }
  ...
  if (pressedWeak != buttonStateWeak) {
    if (now - lastDebounceWeak >= DEBOUNCE_DELAY) {
      ...
      if (buttonStateWeak) {
        modeManual = true;
        currentState = STATE_WEAK;
        Serial.println("Button WEAK pressed: manual WEAK");
      }
    }
  }
  ...
  if (pressedStrong != buttonStateStrong) {
    if (now - lastDebounceStrong >= DEBOUNCE_DELAY) {
      ...
      if (buttonStateStrong) {
        ...
        Serial.println("Button STRONG pressed: manual STRONG");
      }
    }
  }
}
...
void readSensor() {
  ...
  if (!isnan(t)) {
    ...
    if (sensorFailCount >= SENSOR_FAIL_THRESHOLD) {
      ...
      currentState = STATE_WEAK;
      motorPWM = PWM_WEAK;
      Serial.println("Sensor fails exceeded: fallback to WEAK");
    }
  }
}
...
void updateLED() {
  ...
  if (currentState == STATE_IDLE) {
    ...
    // 動作中は常時点灯（LED は updateOutput が管理）
  }
}
